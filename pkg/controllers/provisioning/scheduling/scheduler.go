...
func NewScheduler(ctx context.Context, kubeClient client.Client, nodePools []*v1beta1.NodePool,
	...
	templates := lo.FilterMap(nodePools, func(np *v1beta1.NodePool, _ int) (*NodeClaimTemplate, bool) {
		nct := NewNodeClaimTemplate(np)
		nct.InstanceTypeOptions = filterInstanceTypesByRequirements(instanceTypes[np.Name], nct.Requirements, v1.ResourceList{}).remaining
	...
	s := &Scheduler{
		...
		cachedPodRequests:  map[types.UID]v1.ResourceList{}, // cache pod requests to avoid having to continually recompute this total
		recorder:           recorder,
...
type Scheduler struct {
	...
	cachedPodRequests  map[types.UID]v1.ResourceList // (Pod Namespace/Name) -> calculated resource requests for the pod
